...
export default function Register() {
  ...
  async function registerUser() {
    try {
      ...
      const response = await client.authenticateEmail(email,password,create,username);

  
      // const response = await client.AuthenticateEmailAsync(email, password)
      success("Đăng kí tài khoản thành công")
      navigate("/login")
      console.log("Tài khoản đã được đăng ký:", response);
    } catch (error) {
  ...
  return (
    <div  style={{textAlign:''}}>
      ...
      <div className='mt-5' style={{width:'400px',height:'400px',margin:'auto',backgroundColor:"#EEEEEE"}}>
        <div style={{textAlign:'center',color:"#33CCFF",fontSize:"20px",fontWeight:'bold',marginBottom:"10px"}}>Register</div>
